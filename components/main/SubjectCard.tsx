...
export default function SubjectCard({ subject ,classId }: { subject: Subject, classId: string }) {
  ...
  return (
    <motion.div 
      ...
      className="h-full"
    >
      <Link href={`/home/${classId}/${subject.id}`} className="block group h-full">
        <motion.div 
          whileHover={{ y: -4 }}
          whileTap={{ scale: 0.98 }}
        ...
        >
          {/* Top Gradient */}
          <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-blue-500/80 to-secondary-500/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          ...
          <div className="mt-8 flex items-center justify-between border-t border-border/50 pt-4">
            ...
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-muted text-muted-foreground group-hover:bg-blue-600 group-hover:text-white transition">
