import { Calendar, ChevronRight, ExternalLink, FileText, Video } from "lucide-react"
import { motion } from "framer-motion"
import { Resource } from "@/types"
import { Button } from "../ui/button"
import { useState } from "react"
import DocumentPreview from "./DocumentPreview" // adjust import path as needed
...
export default function ResourceCard({ resource }: { resource: Resource }) {
  ...
  // State for the DocumentPreview sliding pane
  ...
  return (
    <>
      ...
      >
        <div className="block group h-full focus:outline-none">
          ...
          >
            ...
            <div className={`flex items-center gap-3 px-6 py-4 bg-muted/30 border-t border-border/50 mt-auto z-10 transition-colors duration-300 ${isVideo ? 'group-hover:bg-blue-500/5' : 'group-hover:bg-emerald-500/5'}`}>
            {isVideo ? (
              // VIDEO STATE: Single full-width button to watch the video
              <Button
              ...
              <>
                <Button
                  ...
                  className={`flex-1 transition-all duration-300 ${buttonHoverBg} group-hover:text-white group-hover:border-transparent shadow-sm hover:shadow-md`}
                  variant="outline"
                >
      ...
      {/* DocumentPreview rendered outside the card so it can break out of overflow:hidden */}
      {!isVideo && (
        <DocumentPreview
